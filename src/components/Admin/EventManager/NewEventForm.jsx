...
export default function NewEventForm({ eventSubmitUrl, selectedGameId})
{
    ...
    <input disabled={isSubmitting || !selectedGameId || selectedGameId.length === 0} value={prizepool ? prizepool : ""} onChange={(e) => changeprizepool(e.target.value)} type="number" name="prizepool" id="prizepool" className="mx-auto relative text-gray-300 hover:bg-gray-900/70 hover:p-3 h-12 focus:z-[20] focus:outline-none focus:bg-gray-900/70 focus:p-3 outline-none shadow-lg sm:text-sm rounded-xl bg-gray-800/40 duration-200 pl-3"/>
    ...
    <input value={fee? fee : ""} disabled={isSubmitting || !selectedGameId || selectedGameId.length === 0} onChange={(e) => changefeeValue(e.target.value)} type="number" name="fee" id="fee" className="mx-auto relative text-gray-300 hover:bg-gray-900/70 hover:p-3 h-12 focus:z-[20] focus:outline-none focus:bg-gray-900/70 focus:p-3 outline-none shadow-lg sm:text-sm rounded-xl bg-gray-800/40 duration-200 pl-3"/>
    </div>
    ...
    <input value={eventDateTime? eventDateTime : ""} disabled={isSubmitting || !selectedGameId || selectedGameId.length === 0} onChange={(e) => changeEventDateTime(e.target.value)} type="text" name="eventDateTime" className="mx-auto relative text-gray-300 hover:bg-gray-900/70 hover:p-3 h-12 focus:z-[20] focus:outline-none focus:bg-gray-900/70 focus:p-3 outline-none shadow-lg sm:text-sm rounded-xl bg-gray-800/40 duration-200 pl-3"/>
    </div>
